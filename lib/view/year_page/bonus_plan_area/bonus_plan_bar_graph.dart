...
class BonusPlanBarGraph extends HookConsumerWidget {
  const BonusPlanBarGraph({
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    BarGraphStatus barGraphStatus;
    // 入力した値よりグラフの状態を出す
    if (budget == 0) {
      barGraphStatus = BarGraphStatus.noBudget;
    } else if (expense <= budget) {
      barGraphStatus = BarGraphStatus.underBudget;
    } else {
      barGraphStatus = BarGraphStatus.overBudget;
    }
    ...
    return LayoutBuilder(
      builder: ((context, constraints) {
        ...
        double barWidth = degrees <= 1.0
            ? barFrameWidth * degrees
            : barFrameWidth;

        // 予算よりも利用額が少ない場合
        if (barGraphStatus == BarGraphStatus.underBudget) {
          return Stack(
        ...
        // 予算よりも利用額が多い場合
        if (barGraphStatus == BarGraphStatus.overBudget) {
          return Stack(
            alignment: Alignment.centerRight,
            children: [
              // バーの中身
              ...
              SizedBox(
                width:
                    (barFrameWidth * (-lastPrice) / (budget - lastPrice)) *
