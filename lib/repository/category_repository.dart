...
class ImplementsCategoryAccountingRepository
    implements CategoryAccountingRepository {
  @override
  Future<List<CategoryAccountingEntity>> fetchAll({
    ...
    // 各カテゴリーmapでEntity化
    final List<CategoryAccountingEntity> categoryEntityList =
        mapList.map((json) => CategoryAccountingEntity.fromJson(json)).toList();
  ...
  Future<CategoryAccountingEntity> fetchSelectedCategory(
      ...
      required DateTime fromDate,
      required DateTime toDate,
      required int expenseBigCategoryId}) async {
    final sql = '''
                  SELECT
                    t1.${SqfExpenseBigCategory.id} AS id,
                    t1.${SqfExpenseBigCategory.colorCode} AS categoryColor,
    ...
    if (mapList.isEmpty) {
      throw Exception(
          'No data found for the given category ID: $expenseBigCategoryId');
    }
    final categoryEntity = CategoryAccountingEntity.fromJson(mapList.first);
