...
class ImplementsSmallCategoryTileRepository implements SmallCategoryTileRepository {
  ...
  Future<List<SmallCategoryTileEntity>> fetchAll({required int bigCategoryId,required DateTime fromDate,required DateTime toDate}) async {
    ...
    final sql = '''
                SELECT  
                  t1.${TBL201RecordKey().id} AS id,
                  t1.${TBL201RecordKey().displayedOrderInBig} AS displayedOrder,
                  t1.${TBL201RecordKey().categoryName} AS smallCategoryName,
                  coalesce(t2.small_category_payment_sum,0) AS totalExpenseBySmallCategory ,
                  t1.${TBL201RecordKey().bigCategoryKey} AS bigCategoryKey,
                  t1.${TBL201RecordKey().defaultDisplayed} AS defaultDisplayed 
                ...
                LEFT JOIN(
                  ...
                  GROUP BY x.${SqfExpense.paymentCategoryId}
                ) t2
                ON t2.${SqfExpense.paymentCategoryId} = t1.${TBL201RecordKey().id}
                INNER JOIN ${SqfExpenseBigCategory().tableName} t3
                ON t1.${TBL201RecordKey().bigCategoryKey} = t3.${SqfExpenseBigCategory().id}
                WHERE t1.${TBL201RecordKey().bigCategoryKey} = $bigCategoryId
